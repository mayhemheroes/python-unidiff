#!/usr/bin/env bash
#
# mayhem/test.sh — RUN python-unidiff's own test suite (deps already installed by mayhem/build.sh) and
# emit a CTRF (ctrf.io) summary. exit 0 iff failed==0. PATCH-grade oracle: a no-op patch that neuters
# the library breaks the suite's behavioral assertions, so it FAILS here (anti-reward-hacking).
#
# It does NOT compile — build.sh installed pytest + atheris into the in-image site dir and compiled the
# unidiff_run_tests ELF wrapper. We only RUN the suite (unidiff ships unittest TestCases under tests/;
# pytest discovers and runs them). The suite is routed through that compiled NON-system wrapper so the
# gate's sabotage check (neuter non-system binaries to exit(0)) actually perturbs the run (the CPython
...
# Put the in-image site dir (atheris + pytest) and the unidiff source on PYTHONPATH.
